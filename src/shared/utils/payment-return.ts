export type PaymentReturnTarget = { label: string; path: string };
...
export function readPaymentReturnTarget(): PaymentReturnTarget {
  const fallback = { label: "بازگشت به کیف پول", path: "/account/wallet" };

  try {
    const stored = JSON.parse(window.localStorage.getItem(PAYMENT_RETURN_TARGET_KEY) ?? "null") as Partial<PaymentReturnTarget> | null;
    if (!stored || typeof stored.path !== "string" || !stored.path.startsWith("/")) return fallback;
    return {
