...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    console.log("[FCM Push Status on Home]:", {
    ...
    if (permission === "default") {
      ...
      // Chrome requires a user gesture (click/tap) to allow notification popups without suppression.
      // This listener triggers it immediately upon the user's first click anywhere on the page.
      const triggerWithUserGesture = () => {
        if ("Notification" in window && Notification.permission === "default") {
          void requestToken().catch(() => {});
        }
      };

      window.addEventListener("click", triggerWithUserGesture, { once: true });
      window.addEventListener("touchstart", triggerWithUserGesture, { once: true });

      return () => {
        window.removeEventListener("click", triggerWithUserGesture);
        window.removeEventListener("touchstart", triggerWithUserGesture);
      };
    }
