...
import { apiRequest, createEndpoint, getApiAssetUrl } from "./apiClient";
...
export async function getAdvertisementList({
  ...
  const response = await apiRequest<AdvertisementListResponse>(
    createEndpoint("/public/advertise", {
...
export async function getAdvertisementById(id: string): Promise<AdvertisementItem> {
  const response = await apiRequest<AdvertisementShowResponse>(
    `/public/advertise/${id}`,
    {
      authenticated: false,
      method: "GET",
