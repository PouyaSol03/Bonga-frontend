...
type AccessRole = "consultant" | "manager";

const teamConsultants: TeamConsultant[] = [
  {
    id: 1,
    name: "مجتبی مطلبی",
    status: "pending",
...
function AddConsultantRoleOption({
  checked,
  label,
  onClick,
}: {
      ...
      {!isPending && (
        <>
          ...
          <div className="mt-4 grid h-10 grid-cols-3 overflow-hidden rounded-lg bg-[#f5f5f5] text-xs font-medium text-[#4d4d4d]">
            <ConsultantAction icon={<LinearInfoCircle className="h-4 w-4" />} label="اطلاعات" />
            <ConsultantAction icon={<LinearEdit2 className="h-4 w-4" />} label="ویرایش" />
            <ConsultantAction icon={<LinearDelete className="h-4 w-4" />} label="حذف" />
          </div>
        </>
      )}
    </article>
...
function ConsultantAction({
  ...
  return (
    <button
      className="flex items-center justify-center gap-1 border-l border-[#e6e6e6] last:border-l-0"
      type="button"
    >
      {icon}
      {label}
  ...
  return (
    <div className="grid min-h-[420px] place-items-center bg-white px-8 text-center">
      <div className="grid max-w-[260px] justify-items-center">
        ...
        <h2 className="m-0 mt-5 text-sm font-semibold leading-5 text-[#1a1a1a]">
          هیچ مشاوری برای مدیریت وجود ندارد!
        </h2>
        <p className="m-0 mt-2 text-xs font-medium leading-5 text-[#4d4d4d]">
